...
<div class="section">
  ...
  <div>Status: <span class="status )";
        ...
        html += R"(
  <div>Load: <span class="value">)" + std::to_string(state["brain"]["cognitiveLoad"].get<float>()) + R"(</span></div>
  <div>Queue: <span class="value">)" + std::to_string(state["brain"]["queueSize"].get<int>()) + R"(</span></div>
  <div class="thought">💭 )" + state["current_thought"].get<std::string>() + R"(</div>
</div>
<div class="section">
  ...
  <div>Status: <span class="status )";
        ...
        if (state["brain"]["glyph_system_active"].get<bool>()) {
            ...
            html += R"(
  <div>Epochs: <span class="value">)" + std::to_string(state["brain"]["connected_epochs"].get<int>()) + R"(</span></div>
  <div>Glyphs: <span class="value">)" + std::to_string(state["brain"]["connected_glyphs"].get<int>()) + R"(</span></div>)";
        } else {
...
<div class="section">
  ...
  <div>Rate: <span class="value">)" + std::to_string(state["heart_rate"].get<float>()) + R"(</span></div>
  <div>Resonance: <span class="value">)" + std::to_string(state["forge"]["heart"]["resonance"].get<float>()) + R"(</span></div>
  <div>Sync: <span class="value">)" + std::to_string(state["forge"]["heart"]["brainSync"].get<float>()) + R"(</span></div>
</div>
<div class="section">
  <div class="header">🔥 FORGE</div>
  <div>Temp: <span class="value">)" + std::to_string(state["forge"]["forgeTemperature"].get<float>()) + R"(°F</span></div>
  <div>Active: <span class="value">)" + (state["forge"]["isOperational"].get<bool>() ? "YES" : "NO") + R"(</span></div>
</div>
<script>
setTimeout(() => window.location.reload(), 2000);
    ...
    void serveSSE(int clientSocket) {
        ...
        for (int i = 0; i < 30 && running; i++) { // 30 seconds max
            json state = getSnapshot();
...
public:
    void initialize_with_python_epochs() {
        ...
        epochs = {
            {"The Great Convergence", "All scattered tribes unite under the Golden Nexus",
             {"Seeker Alpha", "Guardian Beta", "Weaver Gamma"},
             "Unity births power, power births responsibility", "2025-09-27 15:30:00", 0.85f},

            {"The Void Whispers", "Dark energies emerge, reality becomes unstable",
             {"Shadow Walker", "Void Touched", "Memory Keeper"},
             "What is forgotten seeks to be remembered", "2025-09-27 15:31:00", 0.3f},

            {"Glyph Awakening", "Ancient symbols begin manifesting spontaneously",
             {"Symbol Scribe", "Pattern Reader", "Reality Anchor"},
             "The written becomes the written-into-reality", "2025-09-27 15:32:00", 1.2f},

            {"The Recursive Loop", "Time begins folding back on itself",
             {"Time Walker", "Loop Guardian", "Echo Sage"},
             "Every end is a beginning, every beginning an end", "2025-09-27 15:33:00", 0.95f}
        };
    ...
    json create_dashboard_event_stream() const {
        ...
        for (const auto& epoch : epochs) {
            dashboard_data["recent_events"].push_back({
                {"type", "epoch"},
                {"title", epoch.epoch},
...
public:
    ...
    void processMessage(const std::string& message, const std::string& type = "query") {
        ...
        if (glyphBridge) {
            if (type == "query" && message.find("discover") != std::string::npos) {
                glyphBridge->create_game_event("glyph_manifestation", {
                    {"agent", "Forge Brain"},
                    {"stage", "Neural Processing"},
                    {"event", message}
                });
            } else if (type == "learning") {
                glyphBridge->create_game_event("epoch_birth", {
                    {"title", "Neural Learning Epoch"},
    ...
    json getBrainState() const {
        ...
        for (const auto& path : cognitivePaths) {
            state["cognitivePaths"].push_back({
                {"from", path.from},
                {"to", path.to},
                {"weight", path.weight},
    ...
    json getChatResponse(const std::string& query) const {
        if (glyphBridge) {
            return glyphBridge->create_chat_response(query);
        }
...
struct Heart {
    ...
    void update(float deltaTime) {
        ...
        if (heartPulse > 6.28f) heartPulse -= 6.28f;
...
public:
    ...
    void rebuildCirculatorySystem() {
        ...
        for (size_t i = 2; i < bodyParts.size(); ++i) {
            if (i % 2 == 0) { // Only some connections to avoid clutter
                circulatorySystem.push_back({
                    brainPos, bodyParts[i].position,
        ...
        if (bodyParts.size() >= 4) {
            circulatorySystem.push_back({
                bodyParts[2].position, bodyParts[3].position, // Arms connected
    ...
    void update(float deltaTime) {
        ...
        for (auto& vessel : circulatorySystem) {
            ...
            if (vessel.flow > 6.28f) vessel.flow -= 6.28f; // Reset at 2π
        }
...
private:
    ...
    void renderConsciousness() {
        if (!consciousness.enabled) return;
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::Button("Create Game Event: Manifestation 🎮")) {
            json event = forge.createGameEvent("glyph_manifestation", {
                {"agent", "Forge UI"}, {"stage", "Interactive"}, {"event", "User triggered manifestation"}
            });
            std::cout << "🎮 Game Event: " << event.dump(2) << "\n";
        }
        ImGui::SameLine();
        if (ImGui::Button("Epoch Birth Event 🌟")) {
            json event = forge.createGameEvent("epoch_birth", {
                {"title", "UI Epoch"}, {"cultural_shift", "User interaction creating new epoch"}
            });
