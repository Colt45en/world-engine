...
/************************************
 * Glyph Collation Mapping System
 ************************************/
class GlyphCollationMap {
    constructor() {
        this.glyphToMorpheme = new Map();
        this.morphemeToGlyph = new Map();
        this.unicodeRanges = new Map();
        this.semanticClusters = new Map();
        this.transformationGlyphs = new Map();

        this._initializeBaseGlyphs();
        this._initializeUnicodeRanges();
        this._initializeSemanticClusters();
        this._initializeTransformationGlyphs();
    }

    _initializeBaseGlyphs() {
        // Mathematical symbols mapped to morphemes
        const mappings = [
            // Basic operations
            ['∑', 'multi', 'summation, accumulation'],
            ['∏', 'multi', 'product, multiplication'],
            ['∫', 'build', 'integration, construction'],
            ['∂', 'ing', 'partial derivative, ongoing change'],
            ['∇', 'move', 'gradient, directional change'],
            ['Δ', 'counter', 'delta, difference'],

            // Transformations
            ['⟲', 're', 'rotation, repetition'],
            ['⟳', 're', 'counter-rotation'],
            ['⇄', 'counter', 'bidirectional exchange'],
            ['⇒', 'ize', 'implication, causation'],
            ['⇔', 'ment', 'equivalence, mutual action'],
            ['⊕', 'build', 'direct sum, combination'],
            ['⊗', 'scale', 'tensor product, scaling'],

            // Geometric glyphs
            ['△', 'build', 'triangle, construction'],
            ['□', 'ness', 'square, state containment'],
            ['○', 'move', 'circle, cyclic motion'],
            ['◊', 'ize', 'diamond, transformation'],
            ['⬟', 'multi', 'hexagon, multiplicity'],
            ['⬢', 'scale', 'scaling hexagon'],

            // Negation and opposition
            ['¬', 'un', 'negation'],
            ['∅', 'un', 'empty set, nullification'],
            ['⊥', 'counter', 'perpendicular, opposition'],
            ['∥', 'multi', 'parallel, multiplication'],

            // Greek letters with semantic meaning
            ['α', 'scale', 'alpha scaling factor'],
            ['β', 'move', 'beta movement parameter'],
            ['γ', 'ing', 'gamma ongoing process'],
            ['δ', 'counter', 'delta change operator'],
            ['ε', 'ness', 'epsilon small state'],
            ['ζ', 'ize', 'zeta transformation'],
            ['η', 'ment', 'eta efficiency measure'],
            ['θ', 'move', 'theta rotation angle'],
            ['λ', 'build', 'lambda construction parameter'],
            ['μ', 'scale', 'mu scaling measure'],
            ['ν', 'ing', 'nu frequency, oscillation'],
            ['ξ', 'multi', 'xi multiplicity factor'],
            ['π', 'move', 'pi circular motion'],
            ['ρ', 'ness', 'rho density state'],
            ['σ', 'build', 'sigma construction variance'],
            ['τ', 'ize', 'tau time transformation'],
            ['φ', 'ment', 'phi golden ratio, proportion'],
            ['χ', 'counter', 'chi opposition characteristic'],
            ['ψ', 'ing', 'psi wave function, process'],
            ['ω', 'move', 'omega frequency, ultimate motion'],
        ];

        mappings.forEach(([glyph, morpheme, description]) => {
            this.glyphToMorpheme.set(glyph, { morpheme, description });
            if (!this.morphemeToGlyph.has(morpheme)) {
                this.morphemeToGlyph.set(morpheme, []);
            }
            this.morphemeToGlyph.get(morpheme).push({ glyph, description });
        });
    }

    _initializeUnicodeRanges() {
        // Unicode ranges for different glyph categories
        this.unicodeRanges.set('mathematical', {
            range: [0x2200, 0x22FF],
            name: 'Mathematical Operators',
            defaultMorpheme: 'ize'
        });

        this.unicodeRanges.set('geometric', {
            range: [0x25A0, 0x25FF],
            name: 'Geometric Shapes',
            defaultMorpheme: 'build'
        });

        this.unicodeRanges.set('arrows', {
            range: [0x2190, 0x21FF],
            name: 'Arrows',
            defaultMorpheme: 'move'
        });

        this.unicodeRanges.set('misc_technical', {
            range: [0x2300, 0x23FF],
            name: 'Miscellaneous Technical',
            defaultMorpheme: 'ment'
        });

        this.unicodeRanges.set('greek', {
            range: [0x0370, 0x03FF],
            name: 'Greek and Coptic',
            defaultMorpheme: 'scale'
        });
    }

    _initializeSemanticClusters() {
        // Semantic clusters for contextual morpheme selection
        this.semanticClusters.set('construction', {
            glyphs: ['∑', '∏', '∫', '⊕', '△', '□', 'λ', 'σ'],
            primaryMorpheme: 'build',
            description: 'Construction and building operations'
        });

        this.semanticClusters.set('transformation', {
            glyphs: ['⇒', '⇔', '◊', '⟲', '⟳', 'ζ', 'τ', 'φ'],
            primaryMorpheme: 'ize',
            description: 'Transformation and conversion operations'
        });

        this.semanticClusters.set('motion', {
            glyphs: ['∇', '○', '⇄', 'β', 'θ', 'π', 'ω'],
            primaryMorpheme: 'move',
            description: 'Movement and directional operations'
        });

        this.semanticClusters.set('scaling', {
            glyphs: ['⊗', '⬢', 'α', 'μ'],
            primaryMorpheme: 'scale',
            description: 'Scaling and proportional operations'
        });

        this.semanticClusters.set('negation', {
            glyphs: ['¬', '∅', '⊥'],
            primaryMorpheme: 'un',
            description: 'Negation and nullification operations'
        });

        this.semanticClusters.set('opposition', {
            glyphs: ['Δ', '⊥', 'δ', 'χ'],
            primaryMorpheme: 'counter',
            description: 'Opposition and counteraction operations'
        });

        this.semanticClusters.set('multiplicity', {
            glyphs: ['∏', '⬟', '∥', 'ξ'],
            primaryMorpheme: 'multi',
            description: 'Multiplication and multiplicity operations'
        });

        this.semanticClusters.set('process', {
            glyphs: ['∂', 'γ', 'ν', 'ψ'],
            primaryMorpheme: 'ing',
            description: 'Ongoing processes and continuous operations'
        });

        this.semanticClusters.set('state', {
            glyphs: ['□', 'ε', 'ρ'],
            primaryMorpheme: 'ness',
            description: 'States and properties'
        });

        this.semanticClusters.set('action_result', {
            glyphs: ['⇔', '⊕', 'η', 'φ'],
            primaryMorpheme: 'ment',
            description: 'Actions and their results'
        });
    }

    _initializeTransformationGlyphs() {
        // Special glyphs that modify transformations
        this.transformationGlyphs.set('◈', {
            effect: 'amplify',
            modifier: (button) => {
                const newButton = Object.assign(Object.create(Object.getPrototypeOf(button)), button);
                newButton.alpha *= 1.5;
                newButton.description += ' (amplified)';
                return newButton;
            }
        });
        ...
        this.transformationGlyphs.set('◇', {
            ...
            modifier: (button) => {
                const newButton = Object.assign(Object.create(Object.getPrototypeOf(button)), button);
                newButton.alpha *= 0.7;
                newButton.description += ' (diminished)';
                return newButton;
            }
        });
        ...
        this.transformationGlyphs.set('⟐', {
            ...
            modifier: (button) => {
                const newButton = Object.assign(Object.create(Object.getPrototypeOf(button)), button);
                newButton.M = LLEMath.inverseSmall(button.M);
                newButton.b = LLEMath.vectorScale(button.b, -1);
                newButton.description += ' (inverted)';
                return newButton;
            }
        });
        ...
        this.transformationGlyphs.set('⟡', {
            ...
            modifier: (button) => {
                const newButton = Object.assign(Object.create(Object.getPrototypeOf(button)), button);
                newButton.C = LLEMath.ensureSPD(newButton.C);
                newButton.beta *= 0.5;
                newButton.description += ' (stabilized)';
                return newButton;
            }
        });
    }
    ...
    getMorphemeFromGlyph(glyph) {
        const mapping = this.glyphToMorpheme.get(glyph);
        if (mapping) return mapping.morpheme;

        // Check unicode ranges for fallback
        const codePoint = glyph.codePointAt(0);
        for (const [category, range] of this.unicodeRanges) {
            if (codePoint >= range.range[0] && codePoint <= range.range[1]) {
                return range.defaultMorpheme;
            }
        }
    ...
    getGlyphsForMorpheme(morpheme) {
        return this.morphemeToGlyph.get(morpheme) || [];
    }

    getSemanticCluster(glyph) {
        for (const [clusterName, cluster] of this.semanticClusters) {
            if (cluster.glyphs.includes(glyph)) {
    ...
    createButtonFromGlyph(glyph, label = null, options = {}) {
        const morpheme = this.getMorphemeFromGlyph(glyph);
        if (!morpheme) {
            throw new Error(`No morpheme mapping found for glyph: ${glyph}`);
        }

        const cluster = this.getSemanticCluster(glyph);
        const abbr = options.abbr || glyph;
        const buttonLabel = label || `${glyph}_${morpheme}`;
        const wordClass = cluster ? cluster.description.split(' ')[0] : 'Operator';

        const morphemes = Morpheme.createBuiltInMorphemes(options.dimensions || 3);
        const button = new Button(
            ...
            {
                ...options,
                description: `${glyph} - ${this.glyphToMorpheme.get(glyph)?.description || morpheme}`
            },
            morphemes
        );

        // Apply transformation glyph modifiers if present
        if (this.transformationGlyphs.has(glyph)) {
            return this.transformationGlyphs.get(glyph).modifier(button);
        }

        return button;
    }

    parseGlyphSequence(glyphString) {
        const tokens = [];
        const glyphs = Array.from(glyphString);

        for (let i = 0; i < glyphs.length; i++) {
            const glyph = glyphs[i];
            const morpheme = this.getMorphemeFromGlyph(glyph);
            const cluster = this.getSemanticCluster(glyph);
            const isTransformation = this.transformationGlyphs.has(glyph);
            ...
            tokens.push({
                ...
                isTransformation,
                position: i,
                description: this.glyphToMorpheme.get(glyph)?.description || 'unknown'
            });
        }
    ...
    createButtonSequence(glyphString, options = {}) {
        const tokens = this.parseGlyphSequence(glyphString);
        const buttons = [];

        for (const token of tokens) {
            if (token.morpheme) {
                try {
                    const button = this.createButtonFromGlyph(
                        token.glyph,
                        `${token.glyph}_${token.position}`,
                        options
                    );
                    buttons.push(button);
                } catch (error) {
                    console.warn(`Failed to create button for glyph ${token.glyph}:`, error.message);
                }
            }
        }
    ...
    // Utility methods
    listAllGlyphs() {
        return Array.from(this.glyphToMorpheme.keys());
    }

    listGlyphsByCategory() {
        const categories = {};
        for (const [clusterName, cluster] of this.semanticClusters) {
            categories[clusterName] = cluster.glyphs;
        }
        return categories;
    }

    validateGlyphString(glyphString) {
        const tokens = this.parseGlyphSequence(glyphString);
        const invalid = tokens.filter(t => !t.morpheme);
        return {
            valid: invalid.length === 0,
            invalidGlyphs: invalid.map(t => t.glyph),
            validTokens: tokens.filter(t => t.morpheme).length,
    ...
    exportMappings() {
        return {
            glyphToMorpheme: Object.fromEntries(this.glyphToMorpheme),
            morphemeToGlyph: Object.fromEntries(this.morphemeToGlyph),
            semanticClusters: Object.fromEntries(this.semanticClusters),
            unicodeRanges: Object.fromEntries(this.unicodeRanges),
            transformationGlyphs: Object.fromEntries(
                Array.from(this.transformationGlyphs.entries()).map(([k, v]) =>
                    [k, { effect: v.effect, description: v.effect }]
                )
            )
        };
    }
...
/************************************
 * RNES VectorLab Integration
 ************************************/
class VaultIntegration {
    static async connectToVault() {
        // Check if vault is available
        if (typeof window !== 'undefined' && window.__hiddenDev?.mod?.vault) {
            return window.__hiddenDev.mod.vault;
        }
        return null;
    }

    static async saveGlyphMappingToVault(glyphMap) {
        const vault = await this.connectToVault();
        if (!vault) return { success: false, error: 'Vault not available' };

        try {
            const mappings = glyphMap.exportMappings();
            const data = JSON.stringify(mappings, null, 2);
            await vault.putGlyph('current-mappings', new TextEncoder().encode(data));
            return { success: true, mappings: Object.keys(mappings.glyphToMorpheme).length };
        } catch (error) {
            return { success: false, error: error.message };
        }
    }

    static async loadGlyphMappingFromVault() {
        const vault = await this.connectToVault();
        if (!vault) return null;

        try {
            const buffer = await vault.getGlyph('current-mappings');
            if (!buffer) return null;

            const data = new TextDecoder().decode(buffer);
            return JSON.parse(data);
        } catch (error) {
            console.warn('Failed to load glyph mappings from vault:', error);
            return null;
        }
    }

    static createVaultContext() {
        return {
            LLEMath,
            Morpheme,
            Button,
...
// Make classes globally available for vault integration
if (typeof window !== 'undefined') {
    window.LLEMath = LLEMath;
    window.Morpheme = Morpheme;
    window.Button = Button;
    window.StationaryUnit = StationaryUnit;
    window.ScalingOperations = ScalingOperations;
    window.ButtonFactory = ButtonFactory;
    window.GlyphCollationMap = GlyphCollationMap;
    window.VaultIntegration = VaultIntegration;
    window.MathPro = VaultIntegration.createVaultContext();
}

export { LLEMath, Morpheme, Button, StationaryUnit, ScalingOperations, ButtonFactory, GlyphCollationMap, VaultIntegration };
