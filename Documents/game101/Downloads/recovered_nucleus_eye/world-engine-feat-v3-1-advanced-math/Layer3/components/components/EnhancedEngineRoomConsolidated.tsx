// Enhanced Engine Room - CONSOLIDATED FROM SCATTERED UNTITLED FILES
// This is the MASTER VERSION that combines all your scattered upgrades!
// Features: Clock debugging, WebSocket sync, timing analysis, visual overlays

import React, { useEffect, useRef, useState, useCallback, useImperativeHandle, forwardRef } from 'react';
import { createTier4RoomBridge, Tier4RoomBridge } from '../demos/tier4_room_integration';
import { Tier4State } from './roomAdapter';

// Clock debugging utilities - enhanced window API
declare global {
  interface Window {
    ClockDebug?: {
      ClockBugDetector: any;
      ClockDebugOverlay: any;
    };
  }
}
...
const EnhancedEngineRoom = forwardRef<EnhancedEngineRoomRef, EnhancedEngineRoomProps>(({
  ...
  useEffect(() => {
    if (enableClockDebug && ClockDebugUtils.ClockBugDetector) {
      ...
      log('🕰️ Clock debugging initialized');
    }
  ...
  // Clock tick simulation and monitoring (ENHANCED from scattered code)
  const clockTick = useCallback((timestamp: number) => {
    ...
    // Run clock debugging analysis (ENHANCED)
    ...
    // Send clock sync over WebSocket (ENHANCED)
  ...
  const startClock = useCallback((startBpm?: number) => {
    ...
    clockAnimationRef.current = requestAnimationFrame(clockTick);
    log(`🕰️ Clock started at ${newBpm} BPM`);
  }, [bpm, clockTick, log]);
  ...
  // Initialize the bridge (ENHANCED from scattered code)
  useEffect(() => {
  ...
  const setupBridgeListeners = useCallback((bridge: Tier4RoomBridge) => {
    // Monitor WebSocket connection status (ENHANCED)
    const checkConnection = () => {
      ...
      if (connected !== isConnected) {
        ...
        // Auto-manage clock based on connection
    ...
    // Enhanced event listeners with timing
  ...
  // Expose enhanced methods through ref (CONSOLIDATED)
  useImperativeHandle(ref, () => ({
    ...
    applyOperator: (operator: string, meta?: any) => {
      if (bridgeRef.current) {
        ...
        log('⚠️ Bridge not ready, cannot apply operator:', operator);
      }
    },
    ...
    applyMacro: (macro: string) => {
      if (bridgeRef.current) {
        ...
        log('⚠️ Bridge not ready, cannot apply macro:', macro);
      }
    },
    ...
    // Enhanced clock debugging methods (CONSOLIDATED)
    getClockStats: () => {
